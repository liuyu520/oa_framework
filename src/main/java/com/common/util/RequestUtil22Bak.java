...
import com.common.bean.RestfulParamDto;
import com.google.common.base.Joiner;
import com.kunlunsoft.rewriterequest.web.filter.request.requestfilter.session.CustomFormHttpMessageConverter;
 ...
 */
public class RequestUtil22Bak {
    protected final static Logger logger = Logger.getLogger(RequestUtil22Bak.class);
     ...
     * @return
     */
    public static String parseFormParameters2FormStr(MultiValueMap<String, String> formParameters) {
        Map<String, String> parameterMap = parseFormParameters(formParameters);
    ...
    public static String getRequestUUID(RequestInfoDto requestInfoDto, HttpServletRequest request) {
        StringBuffer stringBuffer = new StringBuffer();
        String seperater = ",";
        stringBuffer.append(requestInfoDto.getServletPath()).append(seperater)
                .append(requestInfoDto.getMethod()).append(seperater)
                .append(requestInfoDto.getFormParameters()).append(seperater)
                .append(requestInfoDto.getRedisCookieId());
        String deviceId = request.getHeader("deviceId");
        if (!ValueWidget.isNullOrEmpty(deviceId)) {
            stringBuffer.append(deviceId);
        }
        String string = stringBuffer.toString();
        return SystemHWUtil.getMD5(string, SystemHWUtil.CHARSET_UTF);
    }


    public static RestfulParamDto getRestfulParamDto(RequestInfoDto requestInfoDto) {
        RestfulParamDto restfulParamDto = new RestfulParamDto();
        BeanHWUtil.copyProperties(requestInfoDto, restfulParamDto);
        restfulParamDto.setParam(requestInfoDto.getFormParameters());
        if (null != requestInfoDto.getParameterMap2()) {
            restfulParamDto.setParamMap(new TreeMap<>(requestInfoDto.getParameterMap2()));
        }

        restfulParamDto.setCookie("conventionk=" + requestInfoDto.getRedisCookieId());
        return restfulParamDto;
    }
